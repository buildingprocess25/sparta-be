...
async function main() {
    const toko = await query(`
        ...
        FROM toko
        WHERE nomor_ulok = $1
        ORDER BY CASE UPPER(TRIM(COALESCE(lingkup_pekerjaan, ''))) WHEN 'SIPIL' THEN 1 WHEN 'ME' THEN 2 ELSE 9 END, id
    `, [ULOK]);
    ...
    const gantt = await query(`
        ...
        WHERE t.nomor_ulok = $1
        GROUP BY g.id, t.id, t.lingkup_pekerjaan
        ORDER BY t.id, g.id
    `, [ULOK]);
    ...
    const checkpointSummary = await query(`
        ...
        ORDER BY CASE UPPER(TRIM(COALESCE(lingkup_pekerjaan, ''))) WHEN 'SIPIL' THEN 1 WHEN 'ME' THEN 2 ELSE 9 END,
                 to_date(tanggal_pengawasan, 'DD/MM/YYYY'), id_pengawasan_gantt
    `, [ULOK]);

    const latestUnfinishedPengawasan = await query(`
        WITH ranked AS (
            SELECT t.lingkup_pekerjaan, g.id AS id_gantt, p.id, pgnt.tanggal_pengawasan,
                   p.kategori_pekerjaan, p.jenis_pekerjaan, p.status, p.catatan, p.dokumentasi,
                   ROW_NUMBER() OVER (
    ...
    const selesaiBelumOpname = await query(`
        WITH latest_selesai AS (
            SELECT t.id AS id_toko, t.lingkup_pekerjaan, g.id AS id_gantt, p.id,
                   pgnt.tanggal_pengawasan, p.kategori_pekerjaan, p.jenis_pekerjaan,
                   p.status, p.catatan, p.dokumentasi,
                   ROW_NUMBER() OVER (
                       PARTITION BY g.id, UPPER(TRIM(COALESCE(p.kategori_pekerjaan, ''))), UPPER(TRIM(COALESCE(p.jenis_pekerjaan, '')))
                       ORDER BY to_date(pgnt.tanggal_pengawasan, 'DD/MM/YYYY') DESC NULLS LAST, p.id DESC
                   ) AS rn
        ...
        ORDER BY CASE UPPER(TRIM(COALESCE(lingkup_pekerjaan, ''))) WHEN 'SIPIL' THEN 1 WHEN 'ME' THEN 2 ELSE 9 END,
                 to_date(tanggal_pengawasan, 'DD/MM/YYYY'), kategori_pekerjaan, jenis_pekerjaan
    `, [ULOK]);
    ...
    const opnameFinal = await query(`
        ...
        WHERE t.nomor_ulok = $1
        GROUP BY ofn.id, t.id, t.lingkup_pekerjaan
    ...
    const opnameItems = await query(`
        ...
        LEFT JOIN instruksi_lapangan_item ili ON ili.id = oi.id_instruksi_lapangan_item
        WHERE t.nomor_ulok = $1
        ORDER BY CASE UPPER(TRIM(COALESCE(t.lingkup_pekerjaan, ''))) WHEN 'SIPIL' THEN 1 WHEN 'ME' THEN 2 ELSE 9 END,
                 kategori_pekerjaan, jenis_pekerjaan, oi.id
    `, [ULOK]);
