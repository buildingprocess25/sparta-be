/**
 * Script untuk menghapus ULOK 1MZ1-2605-0001
 * dan semua data terkait dari database
 * 
 * Data yang akan dihapus:
 * - Entries toko
 * - RAB dengan semua items (CASCADE)
 * - Gantt chart dengan dependencies (CASCADE)
...
async function main() {
  ...
  try {
    ...
    console.log('📋 Memulai proses penghapusan ULOK 1MZ1-2605-0001\n');
    ...
    const tokoResult = await client.query(`
      SELECT 
        ...
        proyek
      FROM toko
      WHERE nomor_ulok = '1MZ1-2605-0001'
      ORDER BY id
    `);
    ...
    const rabResult = await client.query(`
      ...
      WHERE t.nomor_ulok = '1MZ1-2605-0001'
      ORDER BY r.id
    `);
    ...
    const summaryResult = await client.query<DeletionSummary>(`
      WITH deletion_summary AS (
        ...
        FROM toko WHERE nomor_ulok = '1MZ1-2605-0001'
        ...
        WHERE ps.nomor_ulok = '1MZ1-2605-0001'
      )
    ...
    const verifyResult = await client.query(`
      SELECT 
        CASE 
          WHEN EXISTS (SELECT 1 FROM toko WHERE nomor_ulok = '1MZ1-2605-0001')
    ...
    console.log('✅ Penghapusan ULOK 1MZ1-2605-0001 SELESAI\n');
    
    console.log('═══════════════════════════════════════════════════');
    console.log('RINGKASAN AKHIR');
    console.log('═══════════════════════════════════════════════════');
    console.log('✅ ULOK 1MZ1-2605-0001 berhasil dihapus');
