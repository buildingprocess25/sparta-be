...
async function main() {
    ...
    const dateDiff = await query(`
        ...
        SELECT posisi, tanggal_pengawasan
        FROM (
            SELECT 'SIPIL_ONLY' AS posisi, s.tanggal_pengawasan, to_date(s.tanggal_pengawasan, 'DD/MM/YYYY') AS tanggal_sort
            FROM sipil s
            WHERE NOT EXISTS (SELECT 1 FROM me WHERE me.tanggal_pengawasan = s.tanggal_pengawasan)
            UNION ALL
            SELECT 'ME_ONLY' AS posisi, me.tanggal_pengawasan, to_date(me.tanggal_pengawasan, 'DD/MM/YYYY') AS tanggal_sort
            FROM me
            WHERE NOT EXISTS (SELECT 1 FROM sipil s WHERE s.tanggal_pengawasan = me.tanggal_pengawasan)
        ) diff
        ORDER BY posisi, tanggal_sort
    `, [ULOK]);
