...
async function repairSpkKontraktor(spkId: number, correctKontraktor: string) {
    console.log(`[REPAIR] Starting repair for SPK ${spkId}`);
    
    // Update DB
    await pool.query(
        `UPDATE pengajuan_spk SET nama_kontraktor = $1 WHERE id = $2`,
        [correctKontraktor, spkId]
    );
    console.log(`[REPAIR] Updated nama_kontraktor to '${correctKontraktor}' for SPK ${spkId}`);
...
async function main() {
    try {
        ...
        // Data spesifik yang salah berdasarkan hasil query database
        const targets = [
            { id: 1129, namaKontraktor: "CV DUTA PERSATUAN" },
            { id: 1130, namaKontraktor: "CV DUTA PERSATUAN" },
        ];

        for (const target of targets) {
            await repairSpkKontraktor(target.id, target.namaKontraktor);
        }
