...
-- 4a) AUTH_OTP (OTP login untuk Head Office)
...
CREATE TABLE IF NOT EXISTS pic_pengawasan (
    ...
    created_at TIMESTAMP NOT NULL DEFAULT timezone('Asia/Jakarta', now()),
    CONSTRAINT fk_pic_pengawasan_toko_id FOREIGN KEY (id_toko) REFERENCES toko(id) ON DELETE CASCADE,
...
-- Migration safety: tambahkan kolom id_toko dan relasi ke toko jika belum ada.
DO $$
DECLARE
    v_missing_count INT;
    v_invalid_fk_count INT;
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE pic_pengawasan ADD COLUMN id_toko INT;
    END IF;
