...
-- Compatibility migration for testing contractor-first opname against the production DB
-- while production FE/BE still run the legacy code path.
--
-- Important: defaults stay legacy here so old production code can keep inserting
-- pengawasan/opname rows without contractor-first target fields.
...
ALTER TABLE pengawasan_gantt ALTER COLUMN workflow_version SET DEFAULT 'legacy';
