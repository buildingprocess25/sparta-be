...
async function inject() {
    try {
        // 1. Delete dummy hack
        await pool.query(`DELETE FROM pengawasan WHERE catatan = 'HACK_RED_DOT'`);
        ...
        await pool.query(`
            ...
            WHERE id_pengawasan_gantt = 899 AND kategori_pekerjaan = '[IL] INSTALASI'
        `, [imageDocUrl]);
        console.log('Updated 26 Juni (899) to Terlambat');

        // 3. Inject for the rest of the dates
        for (const idPg of targetDates) {
            // Check if already injected
            const check = await pool.query(`SELECT id FROM pengawasan WHERE id_pengawasan_gantt = $1 AND kategori_pekerjaan = '[IL] INSTALASI'`, [idPg]);
            if (check.rowCount > 0) {
            ...
            for (const item of instalasiItems) {
                ...
                `, [
                    550, idPg, '[IL] INSTALASI', item.trim(), 'terlambat',
                    imageDocUrl,
